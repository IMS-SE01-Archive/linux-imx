...
static int imx_sgtl5000_probe(struct platform_device *pdev)
{
	...
	int amp_pwr_en_gpios = -1, amp_en_gpios = -1;
	int ret;
	...
	amp_en_gpios = of_get_named_gpio(np, "amp-en-gpios", 0);
	...
	ret = gpio_request(amp_en_gpios, "amp power enable gpios");
	if(ret)
		printk("[Cifese] request gpio amp_en_gpios fail\n");
	else
		gpio_direction_output(amp_en_gpios, 1);
